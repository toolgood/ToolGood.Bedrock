    ...
    public class CsvFile
    {
                ...
                if (str.StartsWith("\"")) {
                    ...
                    if (str.EndsWith("\"") && !str.EndsWith("\"\""))
                        ...
                        for (var j = i + 1; j < arr.Length; j++) {
                            if (arr[j].EndsWith("\"")) {
                                txt = string.Join(Separator.ToString(), arr.Skip(i).Take(j - i + 1).ToList()).Trim('\"');
        ...
        public void WriteLine(IEnumerable<Object> line)
        {
            ...
            foreach (var item in line) {
                ...
                if (!(item is String str)) str = string.Format("{0}", item);

                if (str.Contains("\""))
                    sb.AppendFormat("\"{0}\"", str.Replace("\"", "\"\""));
                else if (str.Contains(Separator.ToString()) || str.Contains("\r") || str.Contains("\n"))
