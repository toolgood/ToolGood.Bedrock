...
namespace ToolGood.Bedrock.Utils.IdGenerates
{
    public class LongGenerate_1
    {
        ...
        //static LongGenerate_1 snowflake;

        //public static LongGenerate_1 Instance()
        //{
        //    if (snowflake == null)
        //        snowflake = new LongGenerate_1();
        //    return snowflake;
        //}

        public LongGenerate_1()
        {
            Snowflakes(0L, -1);
        }

        public LongGenerate_1(long machineId)
        {
            Snowflakes(machineId, -1);
        }

        public LongGenerate_1(long machineId, long datacenterId)
        {
        ...
        private void Snowflakes(long machineId, long datacenterId)
        {
            if (machineId >= 0) {
                ...
                LongGenerate_1.machineId = machineId;
            }
            if (datacenterId >= 0) {
                if (datacenterId > maxDatacenterId) {
                    throw new Exception("数据中心ID非法");
                }
                LongGenerate_1.datacenterId = datacenterId;
            }
        }
            ...
            lock (syncRoot) {
                ...
                if (lastTimestamp == timestamp) { //同一微妙中生成ID
                    ...
                    if (sequence == 0) {
                        ...
                        timestamp = GetNextTimestamp(LongGenerate_1.lastTimestamp);
                    }
                } else {
                    //不同微秒生成ID
                    sequence = 0L;
